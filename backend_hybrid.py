#!/usr/bin/env python3
"""
 Security System - HYBRID Backend
Combines REAL MQTT Broker + Enhanced  Simulation

Features:
- Real MQTT broker on port 1883 for actual clients
- Enhanced  security device simulation
...
class SecurityBackend:
    ...
    def __init__(self, mqtt_port=1884, websocket_port=8766, http_port=80):
        ...
        logger.info("🏗️   Security Backend initialized (Hybrid Mode)")
    ...
    def _initialize_security_devices(self):
        ...
        for device_id, facility, location, device_type, coords in devices_data:
            ...
            status_weights = [DeviceStatus.ONLINE] * 85 + [DeviceStatus.ALARM] * 10 + [DeviceStatus.OFFLINE] * 5
            ...
            self.security_devices[device_id] = SecurityDevice(
                ...
                status=status,
                coordinates=coords,
                alert_type="TAMPER" if status == DeviceStatus.OFFLINE else ("PRESSURE" if status == DeviceStatus.ALARM else None)
            )

    def _initialize_simulated_clients(self):
        """Initialize 100 simulated BizOptical MQTT clients"""
        client_configs = []
        
        # Generate 100 simple BizOptical device IDs
        for i in range(100):
            # Generate simple sequential device ID like BIZ001, BIZ002, etc.
            device_id = f"BIZ{(i + 1):03d}"
            
            client_id = f"BizOpticID_{device_id}"
            
            # Distribute across different IP ranges for realism
            if i < 50:
                ip_address = f"192.168.1.{100 + i}"
            elif i < 75:
                ip_address = f"10.0.0.{i - 50 + 45}"
            else:
                ip_address = f"172.16.1.{i - 75 + 10}"
            
            topics = [f"BizOpticalMM01S/ID_{device_id}/status", f"BizOpticalMM01S/ID_{device_id}/commands"]
            
            client_configs.append((client_id, ip_address, "optical_device", topics))
    ...
    def _initialize_health_devices(self):
        """Initialize health devices matching the 100 simulated BizOptical clients"""
        # Create health devices for each simulated client (now 100 total)
        for client_id in self.simulated_clients.keys():
            device_id = client_id.replace("BizOpticID_", "")  # Extract device ID like A1B2C3, etc.
            device_name = f"BizOptical {device_id}"
            
            # Start with ONLINE status for all devices initially
            ...
            self.health_devices[device_id] = HealthDevice(
                ...
                name=device_name,
                status=status,
                device_type="BizOptical",
                response_time=random.randint(10, 200) if status == DeviceStatus.ONLINE else 
                           random.randint(500, 2000) if status == DeviceStatus.ALARM else None
            )
        
        logger.info(f"🏥 Initialized {len(self.health_devices)} BizOptical health monitoring devices")
    ...
    async def _send_individual_health_device_update(self, device_id):
        """Send update for a single health device"""
        if device_id not in self.health_devices:
            logger.warning(f"Cannot send update for unknown device: {device_id}")
            return
            
        device = self.health_devices[device_id]
        individual_update = {
            "type": "health_device_update",
            "data": {
                "device_id": device.device_id,
                "name": device.name,
                "status": device.status.value,
        ...
        await self._broadcast_to_websockets(individual_update)
        logger.info(f"📤 Sent individual update for device {device_id}: {device.status.value}")

    async def _handle_device_command(self, websocket, data):
        """Handle BizOptical device control commands"""
        device_id = data.get('device_id')
        command = data.get('command')
        
        if device_id in self.health_devices:
            logger.info(f"Executing command '{command}' on BizOptical device {device_id}")
            
            # Simulate command execution and publish to MQTT
            await self._simulate_mqtt_message(f"BizOpticalMM01S/ID_{device_id}/commands", command, f"BizOpticID_{device_id}")
    ...
    async def _process_real_message(self, message: MQTTMessage):
        ...
        # Extract status from payload if it's a BizOptical status message
        message_status = None
        if message.topic.startswith("BizOptical") and "/status" in message.topic:
            try:
                payload_json = json.loads(message.payload.decode('utf-8', errors='ignore'))
                message_status = payload_json.get('status', '').upper()
            except:
                pass
        ...
        await self._broadcast_to_websockets({
            ...
            'device_status': message_status  # Add status for coloring
        })
    ...
    async def _simulate_mqtt_message(self, topic, payload, client_id="system"):
        ...
        # Extract status from payload if it's a BizOptical status message
        message_status = None
        if topic.startswith("BizOpticalMM01S/") and "/status" in topic:
            try:
                payload_json = json.loads(payload if isinstance(payload, str) else payload.decode('utf-8', errors='ignore'))
                message_status = payload_json.get('status', '').upper()
            except:
                pass
    ...
    async def background_simulation_tasks(self):
        ...
        while self.running:
            try:
                ...
                # Stream health devices updates to all clients
                await self._stream_health_devices_updates()
                
                # Simulate BizOptical MQTT traffic only
                await self._simulate_mqtt_traffic()
                
                await asyncio.sleep(0.5)  # Run every 0.25 seconds for ultra-fast demo
    ...
    async def _update_device_statuses(self):
        ...
        if random.random() < 0.05:  # 5% chance to update a device
            ...
            if random.random() < 0.3:
                ...
                new_status = random.choice([DeviceStatus.ONLINE, DeviceStatus.ALARM, DeviceStatus.OFFLINE])
    ...
    async def _update_health_device_statuses(self):
        """All health devices are now BizOptical devices updated via MQTT messages"""
        # All 100 health devices are now BizOptical devices that get updated via MQTT messages
        # This method is kept for consistency but health devices are primarily updated via MQTT
        pass

    async def _stream_health_devices_updates(self):
        """No automatic updates - only MQTT-triggered device updates"""
        # Completely removed all automatic/random updates
        # ONLY devices that receive actual MQTT messages will update
        # This ensures perfect 1:1 correlation between MQTT message and device update
        pass

    async def _simulate_mqtt_traffic(self):
        """Generate realistic BizOptical MQTT traffic for demonstration"""
        if random.random() < 0.95:  # 95% chance for fast demo
            # Pick a random simulated client
            client_id = random.choice(list(self.simulated_clients.keys()))
            device_id = client_id.replace("BizOpticID_", "")
            
            # Generate BizOptical-style status message
            topic = f"BizOpticalMM01S/ID_{device_id}/status"
            
            # Generate status with 99% ALARM, 1% ONLINE for demo (lots of red moving)
            if random.random() < 0.70:  # 99% chance
                status_choice = "ONLINE"
            else:  # 1% chance
                status_choice = "ALARM"
            ...
            status_data = {
                ...
                "battery_level": random.randint(65, 100) if status_choice != "OFFLINE" else 0,
                "signal_strength": random.randint(-85, -30) if status_choice != "OFFLINE" else -99,
                "temperature": random.randint(18, 35),
                "lock_count": random.randint(0, 50),
                "last_access": (datetime.now() - timedelta(minutes=random.randint(0, 240))).isoformat(),
                "firmware_version": "2.1.3S",  # S for simulated
                "error_code": "00" if status_choice == "ONLINE" else ("ALARM" if status_choice == "ALARM" else "OFFLINE")
            }
            
            payload = json.dumps(status_data)
            await self._simulate_mqtt_message(topic, payload, client_id)
            
            # Update corresponding health device with the status from the MQTT message
            await self._update_health_device_from_mqtt(device_id, status_choice)

    async def _update_health_device_from_mqtt(self, device_id, status_string):
        """Update health device status based on MQTT message"""
        logger.info(f"🔍 Trying to update health device {device_id} with status {status_string}")
        ...
        if device_id in self.health_devices:
            ...
            try:
                new_status = DeviceStatus(status_string.upper())
                device.status = new_status
                
                # Update response time based on new status
                if new_status == DeviceStatus.ONLINE:
                    device.response_time = random.randint(10, 200)
                elif new_status == DeviceStatus.ALARM:
                    device.response_time = random.randint(500, 2000)
                else:  # OFFLINE
                    device.response_time = None
                
                device.last_update = datetime.now()
                
                # If status changed, log and broadcast
                if old_status != new_status:
                    logger.info(f"📱 Health device {device_id} updated from MQTT: {old_status.value} → {new_status.value}")
                    # Send individual device update immediately when MQTT message changes a device
                    await self._send_individual_health_device_update(device_id)
                else:
                    logger.info(f"📱 Health device {device_id} status unchanged: {new_status.value}")
                    
            except ValueError:
                logger.warning(f"Invalid status '{status_string}' received for device {device_id}")
        else:
            logger.warning(f"Device {device_id} not found in health_devices. Available devices: {list(self.health_devices.keys())[:5]}...")
    ...
    async def run(self):
        ...
        logger.info("🚀 Starting  Security System - HYBRID Backend")
    ...
    def _print_connection_info(self):
        ...
        print(f"\n{Fore.GREEN}🚀  Security System - HYBRID Backend Running!{Style.RESET_ALL}")
        print("=" * 60)
        ...
        print("   •  security device simulation")
